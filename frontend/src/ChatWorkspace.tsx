...
interface Props {
  ...
  onSubmit: (value: { prompt: string; screenName: string; platform: string; libraryIds: string[] }) => void;
  onRetry: () => void;
  onCancel: () => void;
}
...
export function ChatWorkspace({
  run, runs, events, busy, settings, onSubmit, onRetry, onCancel,
}: Props) {
  ...
  const history = runs.filter((item) => item.id !== run?.id).slice().reverse();
  const visibleEvents = run?.intent === "chat"
    ? events.filter((event) => event.type === "assistant.message")
    : events.filter((event) => !HIDDEN_CONVERSATION_EVENTS.has(event.type));
  ...
  const submit = () => {
    ...
    onSubmit({
      ...
      platform,
      libraryIds: settings.knowledgeId === "auto" ? [] : [settings.knowledgeId],
    });
    setPrompt("");
  ...
  return (
    <main className="chat-workspace" role="main">
      <section className="conversation" aria-live="polite">
        {!run && history.length === 0 && (
          <div className="empty-conversation">
            ...
            <p>
              Describe your target screen, audience, and key actions. The AI designer
              will turn your requirements into structured, reviewable UI components.
            </p>
            ...
            {item.assistant_message && (
              <article className="message assistant-message">
                ...
                <div className="assistant-body">
                  ...
                  <MarkdownRenderer content={item.assistant_message} />
                </div>
              </article>
            )}
          </div>
        ...
        {visibleEvents.map((event) =>
          event.type === "assistant.message" ? (
            <article className="message assistant-message" key={event.id}>
              <span className="assistant-avatar">
                <Bot size={17} />
              </span>
              <div className="assistant-body">
        ...
        {run?.intent === "chat" && run.assistant_message && !visibleEvents.some((event) => event.type === "assistant.message") && (
          <article className="message assistant-message">
            <span className="assistant-avatar">
              <Bot size={17} />
            </span>
            <div className="assistant-body">
              ...
              <MarkdownRenderer content={run.assistant_message} />
            </div>
          </article>
        )}
      ...
      <div className="composer-wrapper">
        <section className="composer" aria-label="Create design request">
          ...
          <div className="composer-bottom-bar">
            <div className="composer-pills-left">
              <div className="composer-mini-pill screen-pill">
                <Layers size={13} style={{ color: "var(--violet)" }} />
              ...
              {settings.sourceFile && (
                ...
                >
                  <Library size={12} />
                  <span>{settings.sourceFile.split(/[\\/]/).pop()}</span>
                </div>
              )}
            </div>
            ...
            <div className="composer-actions-right">
              {busy && (
                <button className="composer-cancel" onClick={onCancel} type="button">
                  Cancel
                </button>
              )}
              ...
              <button
                ...
                onClick={submit}
                disabled={busy || prompt.trim().length < 3 || !screenName.trim()}
                title={
                  busy
                    ? "Agent is working..."
                    : !screenName.trim()
