...
const WORKFLOW_STAGES = new Set<RunStage>([
  "requirement", "specification", "review_spec", "components", "binding",
  "assembly", "layout_check", "review_final", "finished",
]);
...
export function sessionWorkflowState(
  ...
  const run = activeRun?.intent === "design"
    ? activeRun
    : runs.find((item) => item.intent === "design");
  if (!run) return undefined;

  let stage = WORKFLOW_STAGES.has(run.stage) ? run.stage : run.error?.stage;
  if ((!stage || stage === "blocked" || stage === "failed") && run.id === activeRun?.id) {
    const latest = [...events].reverse().find((event) => event.type === "stage.changed");
